...
const Player: React.FC<PlayerProps> = ({
                ...
                audio.oncanplay = () => {
                    if (currentTrackIdRef.current === trackId && audioRef.current) {
                        ...
                        audioRef.current.play()
                            .then(() => {
                                setIsPlaying(true); // ✅ Updates App State
    ...
    const togglePlay = useCallback((e?: React.MouseEvent) => {
        ...
        setIsPlaying(!isPlaying); // ✅ Calls prop function (App.tsx)
    }, [isPlaying, setIsPlaying]);
    ...
    const handleSkipForward = useCallback((e?: React.SyntheticEvent) => {
        ...
        if (isShuffle) {
            ...
            if (currentShuffledPos === -1) currentShuffledPos = -1;
    ...
    }, [repeatMode, queue, currentTrack, isShuffle, handleSkipForward]);
    ...
    return (
        <>
            {/* ✅ FIXED: Use NATIVE LOOP attribute */}
            <audio
                ref={audioRef}
                loop={repeatMode === 'one'}
                onTimeUpdate={() => !isSeekingRef.current && setProgress(audioRef.current?.currentTime || 0)}
            ...
            {isExpanded ? (
                <div className="md:hidden fixed inset-0 bg-gradient-to-b from-[#505050] to-[#121212] z-[100] flex flex-col h-[100dvh] px-6 pt-safe pb-8 animate-in slide-in-from-bottom duration-300">
                    ...
                    <div className="flex justify-between items-center mb-8 px-2">
                        <button onClick={onToggleShuffle}><Shuffle size={24} className={isShuffle ? "text-[#1ed760]" : "text-white"} /></button>
                        <button onClick={handleSkipBack}><SkipBack size={36} className="text-white fill-white" /></button>
                        <button onClick={togglePlay} className="w-16 h-16 bg-[#1ed760] rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg text-black">
                            {isPlaying ? <Pause size={32} fill="currentColor" /> : <Play size={32} fill="currentColor" className="ml-1" />}
                        </button>
                        <button onClick={handleSkipForward}><SkipForward size={36} className="text-white fill-white" /></button>
                        <button onClick={() => setRepeatMode(prev => prev === 'off' ? 'all' : prev === 'all' ? 'one' : 'off')} className={repeatMode !== 'off' ? "text-[#1ed760]" : "text-white"}>
                    ...
                    <div className="flex justify-between items-center px-4">
                        ...
                        <div className="flex items-center space-x-6">
                            ...
                            <ListMusic size={20} className="text-white" onClick={onToggleQueue} />
                        </div>
                    </div>
                </div>
            ) : (
                ...
                <>
                    <div className="md:hidden fixed bottom-[4.5rem] left-2 right-2 h-14 bg-[#3E3E3E] rounded-md flex items-center px-2 justify-between shadow-lg z-[80]" onClick={() => onToggleExpand()}>
                        ...
                        <div className="flex items-center space-x-3 mr-2">
                            <Heart size={24} className={isLiked ? "fill-[#1ed760] text-[#1ed760]" : "text-white"} onClick={(e) => { e.stopPropagation(); onToggleLike(currentTrack.id); }} />
                    ...
                    <div className="hidden md:flex fixed bottom-0 left-0 right-0 h-24 bg-[#181818] border-t border-[#282828] z-[80] items-center justify-between px-6">
                        ...
                        <div className="flex flex-col items-center w-[40%] space-y-2">
                            <div className="flex items-center space-x-6">
                                <button onClick={onToggleShuffle} className={`hover:text-white transition-colors ${isShuffle ? 'text-[#1ed760]' : 'text-slate-400'}`}><Shuffle size={18} /></button>
                                ...
                                <button onClick={() => setRepeatMode(prev => prev === 'off' ? 'all' : prev === 'all' ? 'one' : 'off')} className={`hover:text-white transition-colors ${repeatMode !== 'off' ? 'text-[#1ed760]' : 'text-slate-400'}`}>
                        ...
                        <div className="flex items-center justify-end w-[30%] space-x-4">
                            ...
                            <button onClick={onToggleQueue} className={`transition-colors ${isQueueVisible ? 'text-[#1ed760]' : 'text-slate-400 hover:text-white'}`}><ListMusic size={18} /></button>
