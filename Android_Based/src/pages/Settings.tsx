...
import { LogOut, User as UserIcon, Moon, Shield, Coffee, Info, Globe, Check, ChevronLeft } from 'lucide-react';
...
const REDIRECT_URI = 'com.nyx.app://callback';
const SCOPES = 'user-read-private user-read-email playlist-read-private playlist-read-collaborative user-top-read user-read-recently-played library-read';
...
const Settings: React.FC<SettingsProps> = ({ onBack }) => {
    ...
    useEffect(() => {
        SpotifyService.getMe().then(setUser).catch(console.error);
    }, []);
    ...
    return (
        <div className="max-w-4xl mx-auto pb-24 animate-in slide-in-from-right duration-300 pt-8 px-4 bg-black min-h-screen">
            ...
            <div className="bg-[#111827]/60 border border-white/5 rounded-3xl p-6 md:p-8 backdrop-blur-sm mb-6">
                <div className="flex items-center space-x-6">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-[#1F2937] overflow-hidden shadow-2xl flex-shrink-0">
                        {/* @ts-ignore - Handle missing images array safely */}
                        {user?.images?.[0]?.url ? (
            ...
            <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 rounded-3xl p-8 border border-white/10 text-center space-y-4">
                ...
                <button
                    onClick={() => window.open('https://buymeacoffee.com/purvesh.dev', '_blank')}
