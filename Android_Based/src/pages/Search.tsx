...
const Search: React.FC<SearchProps> = ({ isAuthenticated, onPlayTrack, onBack, showBack, initialQuery = '' }) => {
    ...
    const openPlaylistModal = async (track: Track, e: React.MouseEvent) => {
        ...
        if (myPlaylists.length === 0) {
            try {
                const res = await SpotifyService.getMyPlaylists();
                setMyPlaylists(res);
            } catch (error) {
    ...
    const addToPlaylist = async (playlistId: string) => {
        ...
        try {
            await SpotifyService.addTracksToPlaylist(playlistId, [selectedTrack.uri]);
    ...
    if (!isAuthenticated) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-6">
                ...
                <button onClick={() => window.location.href = SpotifyService.getAuthorizeUrl()} className="px-8 py-3 bg-blue-600 text-white rounded-full font-bold">Login</button>
            </div>
        );
    }
